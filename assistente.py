...
def clima(cidade):
    ...
    if retorno["cod"] == 200:
        ...
        clima = (
            f"Em {cidade} a temperatura é de"
            f" {str(int(current_temperature - 273.15))} graus celcius e"
            f" humidade de {str(current_humidiy)} %"
        )
...
def tradutor(traducao):
    if traducao == "inglês":
        ...
        cria_audio(
            "traducao.mp3", "O que você gostaria de traduzir para o inglês?"
        )
        ...
        cria_audio(
            "traducao.mp3", "O que você gostaria de traduzir para o português?"
        )
        mensagem = monitora_audio()
        traduzido = traduz.translate(mensagem)
...
def executa_comandos(mensagem):
    ...
    # spotify
        ...
        browser.open(
            "https://open.spotify.com/track/2jvuMDqBK04WvCYYz5qjvG?si=d5118879d68540f3"
        )
    elif "melhor" in mensagem and "banda" in mensagem:
        browser.open(
            "https://open.spotify.com/playlist/37i9dQZF1DZ06evO07zaak?si=2cc1a14c1146467a"
        )
    elif "melhor" in mensagem and "álbum" in mensagem:
        browser.open(
            "https://open.spotify.com/album/4LH4d3cOWNNsVw41Gqt2kv?si=9f6e7d7bcb474666"
        )
    elif "playlist" in mensagem and "rock" in mensagem:
        browser.open(
            "https://open.spotify.com/playlist/5TUxgTIxzLbLVh7RUf9V8i?si=4567c0415d0647b1"
        )
    elif "playlist" in mensagem and "eletronica" in mensagem:
        browser.open(
            "https://open.spotify.com/playlist/2HszJWnlslyuye9GFZQJQc?si=81537070a51d4c97"
        )
    elif "playlist" in mensagem and "brasileira" in mensagem:
        browser.open(
            "https://open.spotify.com/playlist/7ngeDvP8gp3ZtCGfq68jUV?si=49e62791666242a8"
        )
    ...
    # lembrete
        ...
        cria_audio(
            "lembrete.mp3", "O que você gostaria de anotar no lembrete?"
        )
        lembrete = monitora_audio()
        os.system(
            "c: && cd C:/Program Files/Conceptworld/Notezilla &&"
            f' Notezilla.exe /CreateNewNote "{lembrete}"'
        )
    elif "mostrar" in mensagem and "lembrete" in mensagem:
        os.system(
            "c: && cd C:/Program Files/Conceptworld/Notezilla && Notezilla.exe"
            " /BringNotesOnTop"
        )
...
def main():
    cria_audio(
        "ola.mp3",
        "Olá sou o Portela, seu assistente virtual! Como posso ajudar?",
    )
    while True:
        monitora_audio()
